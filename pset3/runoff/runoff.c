...
int find_min(void)
{
   int aux_min_votes = candidates[0].votes;

    for(int i=1;i<candidate_count;i++){

        if(candidates[i].votes < aux_min_votes){
            aux_min_votes = candidates[i].votes;
        }

    }

    return aux_min_votes;
...
bool is_tie(int min)
{
    int number_min_candidates=0;
    for(int i=0;i<candidate_count;i++){

        if(candidates[i].votes == min){
            number_min_candidates++;
        }

    }
    if(number_min_candidates==candidate_count){
        return true;
    }
